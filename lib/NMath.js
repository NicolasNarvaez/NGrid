...
(function() {
  ...
  */
  ...
  mat4.rotatePlane = function(out, v1, v2, theta) {
  ...
  vec4.rotateOnPlane = function(out, v, v1, v2, theta) {
    ...
    if(v1_close || v2_close)
      vec4.rotateNormalized(v1,v2, Math.PI/4);

    //get cuadrant location
    v1_close = vec4.angleDot(tmp_v1, v1) < Math.PI/2;
    v2_close = vec4.angleDot(tmp_v1, v2) < Math.PI/2;
  ...
  vec4.angleDot = function(va, vb) {
    ...
    (Math.sqrt( va[0]*va[0] + va[1]*va[1] + va[2]*va[2] + va[3]*va[3] ) * Math.sqrt( vb[0]*vb[0] + vb[1]*vb[1] + vb[2]*vb[2] + vb[3]*vb[3] ));
  }
  ...
  vec4.rotatePivot = function(out, v, p, theta) {
    ...
    vec4.rotateNormalized(v_norm, pivot, theta);
  ...
  global.NMath = (function() {
    ...
    */
    ...
    common = (function() {
      ...
      function copy(out, from, length, offset, offset_out) {
        if(!offset) offset = 0;
        //if(!offset_out)
        //if(!length) length =
      };
