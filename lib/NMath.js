...
(function() {
  ...
  */
  ...
  mat4.rotationPlane = function(out, v1, v2, theta) {
    var tmp_v1 = new vec4.create();
    //console.log('plano: ', v1, ', ', v2)

    vec4.rotateOnPlane(tmp_v1, [1,0,0,0], v1, v2, theta);
    NMath.common.copy(out, tmp_v1, 0, 4);

    //console.log('\n\n');

    vec4.rotateOnPlane(tmp_v1, [0,1,0,0], v1, v2, theta);
    NMath.common.copy(out, tmp_v1, 4, 4);

    //console.log('\n\n');

    vec4.rotateOnPlane(tmp_v1, [0,0,1,0], v1, v2, theta);
    NMath.common.copy(out, tmp_v1, 8, 4);

    //console.log('\n\n');

    vec4.rotateOnPlane(tmp_v1, [0,0,0,1], v1, v2, theta);
    NMath.common.copy(out, tmp_v1, 12, 4);

  };

  //perpendiculariza y normaliza
  vec4.plane = function(va, vb) {
    var tmp_v = vec4.create();
    vec4.projection(tmp_v, va, vb);
    vec4.sub(va, va, tmp_v);
    vec4.normalize(va, va);
    vec4.normalize(vb, vb);
  }

  vec4.ortogonalize = function(vecs) {
    var v_tmp = new vec4.create();

    vec4.projection(v_tmp, vecs[0], vecs[1]);
    vec4.sub(vecs[0], vecs[0], v_tmp);
    vec4.projection(v_tmp, vecs[0], vecs[2]);
    vec4.sub(vecs[0], vecs[0], v_tmp);
    vec4.projection(v_tmp, vecs[0], vecs[3]);
    vec4.sub(vecs[0], vecs[0], v_tmp);


    vec4.projection(v_tmp, vecs[1], vecs[2]);
    vec4.sub(vecs[1], vecs[1], v_tmp);
    vec4.projection(v_tmp, vecs[1], vecs[3]);
    vec4.sub(vecs[1], vecs[1], v_tmp);

    vec4.projection(v_tmp, vecs[2], vecs[3]);
    vec4.sub(vecs[2], vecs[2], v_tmp);
  }
  ...
  */
  ...
  vec4.rotateOnPlane = function(out, v, v1, v2, theta) {
    ...
    //fix superposed vectors
    vec4.sub(tmp_v2, tmp_v1, v1);
    v1_close = vec4.length(tmp_v2) < 0.05;
    vec4.sub(tmp_v2, tmp_v1, v2);
    v2_close = vec4.length(tmp_v2) < 0.05;
    if(v1_close || v2_close)
      vec4.rotateNormalized(v2,v1, Math.PI/4);

    //console.log('rotados superpuestos',v1, v2);
    //get cuadrant location
    v1_close = vec4.angleDot(tmp_v1, v1) >= 0;
    v2_close = vec4.angleDot(tmp_v1, v2) >= 0;

    //choose right pivot for given cuadrant
    if( v1_close && v2_close)
      vec4.copy(tmp_v2, v2);
    if( v1_close && !v2_close)
      vec4.copy(tmp_v2, v1);
    if( !v1_close && v2_close )
      vec4.scale(tmp_v2, v1, -1);
    if( !v1_close && !v2_close )
      vec4.scale(tmp_v2, v2, -1);
    //console.log('v1_close: ', v1_close, ' angle: ',vec4.angleDot(tmp_v1, v1),
    //  ', v2_close: ', v2_close, 'angle: ', vec4.angleDot(tmp_v1, v2));
    //console.log('pivote',tmp_v2);
    vec4.rotatePivot(tmp_v1, tmp_v1, tmp_v2, theta);
    //console.log('rotado',tmp_v1);
    vec4.add(out, v_perpendicular, tmp_v1);
    //console.log('final',out);

    //console.log('longitud',vec4.length(out));
  }

  vec4.angleDot = function(va, vb) {
    //dot / a.leng*b.length
    return Math.acos(
      ( va[0]*vb[0] + va[1]*vb[1] + va[2]*vb[2] + va[3]*vb[3] )/

    (Math.sqrt( va[0]*va[0] + va[1]*va[1] + va[2]*va[2] + va[3]*va[3] ) *
    Math.sqrt( vb[0]*vb[0] + vb[1]*vb[1] + vb[2]*vb[2] + vb[3]*vb[3] ))
  ...
  vec4.rotateAbsolute = function(obj) {
    var mat5 = NMath.mat5,
      mat = NMath.mat,
      rwz = mat5.create(),
      rwy = mat5.create(),
    ...
    mat5.rotateWZ(rwz, obj.rwz);
    mat5.rotateWY(rwy, obj.rwy);
    mat5.rotateWX(rwx, obj.rwx);

    // twy*twz*twx*m_data
    mat5.multiply(m_tmp1, rwz, rwx);
    //mat5.sum(m_tmp1, rwz, rwx); //WTF : (TODO: entender xd)
    mat5.multiply(m_tmp2, rwy, m_tmp1);
    mat5.multiply(m_tmp1, m_tmp2, m_data);

    mat.createFrom(m_data_pre, m_tmp1);
    vecs = mat.vecs(m_data_pre);
  ...
  vec4.rotateNormalizedRelative = function(vecs, rotation ,theta) {
    var mat = NMath.mat,
      mat5 = NMath.mat5,
      vec = NMath.vec,
      m_data = mat5.create(),
    ...
    rotation(m_rotation, theta);

    mat5.multiply(m_tmp, m_rotation, m_data);
    vec.fromMat(vecs, m_tmp);
  }

  mat4.multiplyVecInverse = function(out, m, v) {
    mat4.multiplyVec(out, m, v);
    vec4.flip(out, v);
  }

  //rota v hacia p theta ang
  vec4.rotatePivot = function(out, v, p, theta) {
  ...
  //genera la proyeccion de va en el eje dado por vb
  ...
  */
  vec4.flip = function(out, va, vb) {
    ...
    out[0] = va[0] +2*( vb[0]*relation - va[0] );
    out[1] = va[1] +2*( vb[1]*relation - va[1] );
    out[2] = va[2] +2*( vb[2]*relation - va[2] );
    out[3] = va[3] +2*( vb[3]*relation - va[3] );
  }
    ...
    */
    mat5 = (function() {
      ...
      */
      ...
      function rotate(rotation, rotation_f, theta, relative) {
        rotation_f(tmp_m_1, theta);
        if(relative)
          mat5.multiply(tmp_m_2, rotation, tmp_m_1);
        else
          mat5.multiply(tmp_m_2, tmp_m_1, rotation);
        mat.from(rotation, tmp_m_2);
      };
      ...
      */
      ...
      function multiplyVecInverse(out, m, v) {
        mat5.multiplyVec(out, m, v);
...
//sum
var x, y, length=5, str='';
for(x=length; x--;)
  for(y=length; y--;)
    str += 'out['+(x*length+y)+']=a['+(x*length+y)+']+b['+(x*length+y)+'];\n'
