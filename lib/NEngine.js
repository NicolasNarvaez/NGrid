  ...
  this.NEngine = (function() {
    ...
    math = (function() {
      ...
      function rotateNormalized(va, vb, theta) {
        ...
        vec4.scaleAndAdd(va, va, va, Math.cos(theta)-1);
      ...
      function render() {
        ...
        if(obj_list.length) {
          ...
          for(obj_list_i=obj_list.length; obj_list_i--;) {
            ...
            //left draw
            vec4.copy(tmp_vec1, camera3.rz);
            vec4.copy(tmp_vec2, camera3.rx);
            math.rotateNormalized(tmp_vec1, tmp_vec2, -Math.PI/60);

            mat4.rotationVecs(m_cameraRotation3, tmp_vec2, camera3.ry, tmp_vec1);
            mat4.translate(tmp_matrix4, m_cameraRotation3, [2.616,0,0,0]);
