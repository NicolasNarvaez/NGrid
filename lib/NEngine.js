    ...
    renderer = (function() {
      ...
      function init(options) {
        ...
        if(options.projection_automatic) {
          ...
          mat4.perspective(uPMVMatrix3D, options.projection_3d_angle,
        ...
        if(obj_list.length) {
          ...
          for(obj_list_i=obj_list.length; obj_list_i--;) {
            ...
            if(options_init.projection_automatic) {
              ...
              mat4.perspective(uPMVMatrix3D, options_init.projection_3d_angle,
                canvas.width/canvas.height,
            ...
            /*
            attrib_vertex = context.getAttribLocation(shader_info, "a_position");
          	context.enableVertexAttribArray(attrib_vertex);
          	context.bindBuffer(context.ARRAY_BUFFER, obj_list_obj.geom.buffers_info.position);
          	context.vertexAttribPointer(attrib_vertex, 4, context.FLOAT, false, 0, 0);

          	attrib_color = context.getAttribLocation(shader_info, 'a_color');
          	context.enableVertexAttribArray(attrib_color);
          	context.bindBuffer(context.ARRAY_BUFFER, obj_list_obj.geom.buffers_info.color);
          	context.vertexAttribPointer(attrib_color, 4, context.FLOAT, false, 0, 0);

          	context.bindBuffer(context.ELEMENT_ARRAY_BUFFER, obj_list_obj.geom.buffers_info.indices);
          	context.drawElements(context.LINES, obj_list_obj.geom.buffers_info.number_vertex_points, context.UNSIGNED_SHORT, 0);
            */
          }
        }
      }
      function makeShader(src, type) {
      	src = document.getElementById(src).text; //obtener src
      	...
      	if(!context.getShaderParameter(shader, context.COMPILE_STATUS) )
      		alert("Error compilando el shadder:"+context.getShaderInfoLog(shader));
      ...
      function initShaders() {
      	//compilar shaders
      	vertexShader = makeShader('vs', context.VERTEX_SHADER);
      	fragmentShader = makeShader('fs', context.FRAGMENT_SHADER);
      	//crear, enlazar con shaders, usar, programa
      	shader_info = context.createProgram();
      	context.attachShader(shader_info, vertexShader);
      	context.attachShader(shader_info, fragmentShader);
      	context.linkProgram(shader_info);

      	if(!context.getProgramParameter(shader_info, context.LINK_STATUS))
      		alert("No se pudo inicializar el programa de shaders");

      	context.useProgram(shader_info);
      };
      function getMatrixUniforms() {
      	shader_info.uPMVMatrix1Uniform = context.getUniformLocation(shader_info, "uPMVMatrix1");
      	shader_info.uPMVMatrix2Uniform = context.getUniformLocation(shader_info, "uPMVMatrix2");
      }
      function setMatrixUniforms() {
      	context.uniformMatrix4fv(shader_info.uPMVMatrix1Uniform, false, uPMVMatrix1);
      	context.uniformMatrix4fv(shader_info.uPMVMatrix2Uniform, false, uPMVMatrix2);
      }
      function objAdd(obj) {
        if(obj.geom && obj.geom.buffers_info !== null) {
          obj.geom.buffers_info = twgl.createBufferInfoFromArrays(context, obj.geom.buffers );
          /*
          obj.geom.buffers_info = {
            position : context.createBuffer(),
            color : context.createBuffer(),
            indices : context.createBuffer()
          };
          console.log(obj.geom);
          context.bindBuffer(context.ARRAY_BUFFER, obj.geom.buffers_info.color );
          context.bufferData(context.ARRAY_BUFFER, obj.geom.buffers.color.data, context.STATIC_DRAW);

          context.bindBuffer(context.ARRAY_BUFFER, obj.geom.buffers_info.position );
          context.bufferData(context.ARRAY_BUFFER, obj.geom.buffers.position.data, context.STATIC_DRAW);

          obj.geom.buffers_info.indices.number_vertex_points = obj.geom.buffers.indices.length;
          context.bindBuffer(context.ELEMENT_ARRAY_BUFFER, obj.geom.buffers_info.indices);
          context.bufferData(context.ELEMENT_ARRAY_BUFFER, obj.geom.buffers.indices.data, context.STATIC_DRAW);
          */
        }
        obj_list.push(obj);
      }
