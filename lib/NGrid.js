...
(function() {
  var global = this;
    ...
    function init(options) {
        ...
        for(var i_z=size;i_z--;)
          for(var i_y=size;i_y--;)
            for(var i_x=size;i_x--;) {
              ...
              //camera.p = grid.p;
              grid = new Obj();
              grid.geom = NEngine.geometry.grid4(2,2,2,2, {size: 0.2});
      ...
      grid = new Obj();
      grid.geom = NEngine.geometry.grid4(4,4,4,4, {size:1000});
      grid.p[3] = 0;
      renderer.objAdd(grid);
    ...
    var iterators = [],
      enemigos = new NEngine.obj.list();

    iterators.push(enemigos);

    enemigos.pases.push(function(list) {
      var tmp_v1 = vec4.create(),
        tmp_v2 = vec4.create();

      for(var i = list.length; i--;) {
        vec4.copy(tmp_v1, list[i].p);
